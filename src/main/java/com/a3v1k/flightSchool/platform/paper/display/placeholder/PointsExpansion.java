...
public class PointsExpansion extends PlaceholderExpansion {
    ...
    public String onPlaceholderRequest(Player player, @NotNull String params) {
        if (params.equalsIgnoreCase("timer")){
            GameState state = this.plugin.getGameManager().getGameState();

            if (state == GameState.ROLE_SELECTION) {
                long startedAt = this.plugin.getGameManager().getGameStartedAt();
                if (startedAt == -1) return "Starting Soon!";

                long elapsedSeconds = (System.currentTimeMillis() - startedAt) / 1000;
                long remaining = Math.max(0, 20 - elapsedSeconds);
            ...
            if (state == GameState.IN_GAME) {
                ...
                int matchDuration = this.plugin.getConfigManager().getMatchDuration();
                long elapsedSeconds = (System.currentTimeMillis() - startedAt) / 1000;
                long remaining = Math.max(0, matchDuration - elapsedSeconds);
