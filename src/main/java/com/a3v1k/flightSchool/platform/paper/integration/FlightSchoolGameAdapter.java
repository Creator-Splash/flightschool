...
import creatorsplash.creatorsplashcore.api.*;
...
public final class FlightSchoolGameAdapter implements GameAdapter {
    ...
    public void onGameStart(GameContext ctx) {
        ...
        GameOrchestrator orchestrator = plugin.getGameOrchestrator();
    ...
    public void onPlayerArrive(Player player, String teamName, GameContext ctx) {
        ...
        if (fsTeam == null) {
            plugin.getLogger()
                .warning("[FlightSchoolGameAdapter] Player %s arrived with no resolvable team (event team='%s')."
                    .formatted(player.getName(), teamName));
            return;
        }
        ...
        if (state != GameState.IN_GAME && state != GameState.CINEMATIC) return;
    ...
    private void seedTeamsFromContext(GameContext ctx) {
        ...
        for (Map.Entry<String, List<UUID>> entry : ctx.teamAssignments().entrySet()) {
            ...
            for (UUID id : entry.getValue()) {
                ...
                if (p == null) continue;
                gm.assignPlayerToTeam(p.getUniqueId(), fsTeam);
            }
        }
    }
    ...
    private void scheduleEvacuation() {
        plugin.getScheduler().runLater(() ->
            CreatorSplashCore.evacuateToLobby(
